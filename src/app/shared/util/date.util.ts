 ...
 *
 * Трата привязана к дню, а не к моменту времени, и сервер отдаёт её дату
 * календарной строкой dd.MM.yyyy уже в поясе владельца. Разбор идёт по
 * календарной части строки, без `new Date(...)`: тот трактует ISO-момент по
 * поясу браузера и в поясах западнее Гринвича сдвигает дату на предыдущие
