...
// лишние, но пришедшее с ними значение читается. Доли секунды приходят с
// сервера: swagger у API нет, и сериализатор волен отдать «10:00:00.0000000».
const EXACT_TIME = /^(\d{2}):(\d{2})(?::\d{2}(?:\.\d+)?)?$/;
...
/**
 * Время суток для подписи: те же HH:mm, что и в полях ввода.
 *
 * Сервер шлёт одно и то же поле и как «10:00», и как «10:00:00», поэтому
 * подпись нормализует значение сама. Нечитаемое время показывается как есть:
 * подпись останется непривычной, но не потеряет то, что пришло. Пустое
 * значение отдаётся пустым - подпись такую часть просто опускает.
 */
export function formatTimeLabel(value: string | null | undefined): string {
  ...
  return parseTime(value) ?? value.trim();
}
