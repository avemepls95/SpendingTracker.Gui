...
export class CategoriesTreeComponent implements OnInit {
    ...
    _filter(category: any): Category[] {
        if (this.treeCategories.find(c => c.id == category.id)){
            return this.categoriesForSelect;
        }

        const filterValue = category ? category.title.toLowerCase(): '';

        let ids = this.treeControl.dataNodes.map(c => c.id);
        let result = this.categoriesForSelect.filter(category =>
    ...
    addParent(categoryNode: FlatNode) {
    ...
    createParentCategoryAndAddToParent(newParentTitle: string, childId: string) {
      this.loaderService.show();
      this.spendingApiService.createParentCategoryAndAddToParent(newParentTitle, childId)
    ...
    cancelAddingToParent(childId: string) {
